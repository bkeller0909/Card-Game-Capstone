...
public class GameDebugger : MonoBehaviour
{
    ...
    [Header("All Ring Spells Being used at the moment")]
    public int ThornsOfAgony = 0;
    public int GuardiansTouch = 0;
    public int ManaMerchant = 0;
    public int SpectralChain = 0;
    public int VampiricSurge = 0;
    public int VeilOfFortitude = 0;
    public int VengfulMirror = 0;
    #endregion
    ...
    private void TrackSpellsAvailable()
    {
        foreach (GameObject cardsInUse in CardsObjectPool.Instance.objPoolCards)
        {
            ...
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.GuardiansTouch)
            {
                GuardiansTouch = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.ManaMerchant)
            {
                ManaMerchant = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.SpectralChain)
            {
                SpectralChain = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.VampiricSurge)
            {
                VampiricSurge = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.VeilOfFortitude)
            {
                VeilOfFortitude = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
            else if (cardsInUse.GetComponent<SpellCard>().spellName == SpellNames.VengefulMirror)
            {
                VengfulMirror = CardsObjectPool.Instance.allcardAmounts[(int)cardsInUse.GetComponent<SpellCard>().spellName];
            }
        }
    }
        ...
        foreach (SpellNames attackSpell in listOfPreference)
        {
            foreach (GameObject card in CardsObjectPool.Instance.objPoolCards)
            {
                if (card.GetComponent<SpellCard>().spellName == attackSpell && !card.activeSelf)
                {
                    ...
                    card.transform.GetChild(0).gameObject.layer = 7;
                    for (int i = 0; i < cardHandP1.emptySlots.Length; i++)
                    {
        ...
        foreach (SpellNames attackSpell in listOfPreference)
        {
            foreach (GameObject card in CardsObjectPool.Instance.objPoolCards)
            {
                if (card.GetComponent<SpellCard>().spellName == attackSpell && !card.activeSelf)
                {
                    ...
                    card.transform.GetChild(0).gameObject.layer = 6;
                    for (int i = 0; i < cardHandP2.emptySlots.Length; i++)
                    {
